...
exports = module.exports = (
  ...
  // Liveness: point the platform's health check here. NOT a bare "the process is
  // up" probe — that answer is always yes even when the crawlers have silently
  // stopped, which is this service's actual failure mode. It reports 503 when a
  // background loop has gone quiet (see lib/health.js), so the platform recycles
  // the instance instead of leaving it up and idle. Registered before the
  // catch-all so it isn't treated as a URL to render, and unauthenticated (DO's
  // health_check can't send the token) — detail is token-gated, the verdict isn't.
  app.get('/health', (req, res) => {
    const full = health.report(server);
    res
      .status(full.ok ? 200 : 503)
      .json(cacheAuthOk(req) ? full : health.summarize(full));
  });
