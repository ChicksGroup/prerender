...
exports = module.exports = (
  ...
  // TTL) the lazy paths haven't cleared. One bounded sweep per call (maxScan /
  // maxEvict); cluster-locked server-side, so a concurrent call returns
  // { skipped:true }. Loop it (until { reaped:0 }) to drain a backlog. The
  // scheduled refresher also runs this on a timer (REFRESHER_REAP_INTERVAL_MS).
  app.post('/cache/reap', bodyParser.json(), (req, res) => {
    ...
    redisCache
      .reapExpired({
        maxScan: num(b.maxScan != null ? b.maxScan : req.query.maxScan),
