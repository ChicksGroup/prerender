...
exports = module.exports = (
  ...
  // Producer endpoint: the cache-manager pushes seed/refresh URLs here at a
  // priority (0 = highest); the in-server refresher consumes them at capacity.
  ...
  // Recent fallback-FAILURE events (capped ring buffer): which pages fell back to
  // the SaaS and the fallback itself failed, why, and when. ?limit=N (default 200).
  // The dashboard's "Fallbacks" page ingests these into MySQL for durable analysis.
  app.get('/cache/fallback-failures', (req, res) => {
    if (!cacheAuthOk(req)) return res.sendStatus(403);
    redisCache
      .fallbackFailures({ limit: req.query && req.query.limit })
      .then((p) => res.json(p))
      .catch((e) => res.status(503).json({ error: e.message }));
  });
