...
function index(req, res) {
  ...
  return Promise.resolve()
    .then(() => fetchFn(indexApiUrl(code)))
    .then((xml) => sendXml(res, renderIndex(host, parseSitemapLocs(xml))))
    .catch((e) => {
      util.log('[sitemap] index error', host, e && e.message);
...
function page(req, res, n) {
  ...
  return Promise.resolve()
    ...
    .catch((e) => {
      util.log('[sitemap] page error', host, n, e && e.message);
      return res.sendStatus(502);
    });
}
