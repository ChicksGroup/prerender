...
function parseConfig() {
  ...
  const apiBase = (process.env.SITEMAP_API_BASE || 'https://api.chicksgroup.com').replace(/\/+$/, '');
  return {
    ...
    apiBaseByEnv: {
      prod: apiBase,
      dev: (process.env.SITEMAP_API_BASE_DEV || 'https://dev-api.chicksgroup.com').replace(/\/+$/, ''),
      staging: (process.env.SITEMAP_API_BASE_STAGING || 'https://staging-api.chicksgroup.com').replace(/\/+$/, ''),
    },
    shortcodes: map,
    hostByCode: byCode,
...
function index(req, res) {
  ...
  return Promise.resolve()
    .then(() => fetchFn(indexApiUrl(site.apiBase, site.code)))
