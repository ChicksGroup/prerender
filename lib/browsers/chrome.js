...
chrome.setUpEvents = async function (tab) {
  ...
  Network.responseReceived((params) => {
    ...
    if (
      ...
      // 5XX. Name the failing subresource (once, on the transition) so an operator
      // can see WHICH backend dragged the page into a dirty render — this is what
      // makes isCacheable store-skip the page (reason=dirtyRender) even though the
      // top-level render returns 200, so the page silently never re-caches.
