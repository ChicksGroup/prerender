...
// stays critical (the legacy behavior); DIRTY_RENDER_ALL_HOSTS=true forces that
// legacy behavior even with a whitelist configured.
...
function isCriticalSubresource(subresourceUrl, pageUrl) {
  ...
  const criticalDomains = parseDomainList(process.env.ALLOWED_DOMAINS).concat(
    parseDomainList(process.env.DIRTY_RENDER_EXTRA_DOMAINS),
  );
  if (criticalDomains.length === 0) return true;
  ...
  return criticalDomains.some((d) => subHost === d || subHost.endsWith('.' + d));
}
...
chrome.setUpEvents = async function (tab) {
  ...
  Network.responseReceived((params) => {
    ...
    if (
      ...
      if (isCriticalSubresource(params.response.url, tab.prerender.url)) {
        // 5XX on one of OUR backends. Name the failing subresource (once, on the
        // transition) so an operator can see WHICH backend dragged the page into
        // a dirty render — this is what makes isCacheable store-skip the page
        // (reason=dirtyRender) even though the top-level render returns 200, so
        // the page silently never re-caches.
        if (!tab.prerender.dirtyRender) {
          util.log(
            '[dirtyRender]',
            JSON.stringify({
      ...
      } else if (!tab.prerender.dirtyIgnoredLogged) {
        // Third-party 5xx: keep the operator signal (once per render) but don't
        // fail the store — a broken extra must not stop OUR page from caching.
        tab.prerender.dirtyIgnoredLogged = true;
        util.log(
          '[dirtyRender]',
          JSON.stringify({
            page: tab.prerender.url,
