  ...
  return new Promise((resolve) => {
    ...
    const req = lib.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        // The fallback itself failed — don't serve/cache a SaaS error.
        if (status >= 500 || status === 429) return resolve(null);
        const headers = {};
    ...
    return fetchFallback(req.prerender.url)
      .then((result) => {
        if (result && typeof result.content === 'string' && result.content) {
          ...
          logEvt('fallback-failed', { url: req.prerender.url, after: code });
        }
        next();
      })
      .catch(() => next());
