...
function launch(url) {
  ...
  Promise.resolve()
    ...
    .then((status) => {
      ...
      util.log('[refresher]', JSON.stringify({ evt: 'refresh', url: url, status: status, ms: ms }));
    })
    .catch((e) => {
      recordOutcome({ ok: false, latencyMs: Date.now() - t0 });
      util.log('[refresher]', JSON.stringify({ evt: 'refresh-error', url: url, err: e && e.message }));
    })
...
function recordOutcome(o) {
  ...
  if (!o || o.ok === false || (status != null && status >= 500)) winFailed += 1;
  else winCompleted += 1;
}
