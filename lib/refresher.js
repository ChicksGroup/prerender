...
// Park a failed URL so dueForRefresh skips it until the cooldown elapses; a real
// result clears any prior parking. A render is a FAILURE worth parking when it
// timed out / 5xx'd / threw — NOT when it was shed for backpressure (429/503),
// which says "the box was busy", not "this URL is broken". Per-instance (not in
// Redis): the refresher runs on the dedicated scheduled tier — typically a single
// instance — and dueForRefresh has no cross-instance dedup anyway, so a shared
// park set would buy little. Revisit if that tier ever scales horizontally.
...
function launch(url, source) {
  ...
  Promise.resolve()
    ...
    .then((status) => {
      ...
      if (status == null || (status >= 500 && status !== 503)) noteFailure(url);
      else if (status !== 429 && status !== 503) clearFailure(url);
