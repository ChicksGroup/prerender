...
function parseConfig() {
  const tickMs = num(process.env.SEEDER_TICK_MS, 60000);
  const fetchTimeoutMs = num(process.env.SEEDER_FETCH_TIMEOUT_MS, 30000);
  return {
    ...
    // Deep-liveness threshold (consumed by lib/health.js). Like the refresher,
    // this loop is a self-rescheduling setTimeout chain, so a tickOnce() that
    // never settles ends it with nothing logged and no crash — the box stays up
    // and simply never seeds again. beat() is called at the tick AND at every
    // checkpoint inside a scan, so a legitimately long scan (many sitemaps, one
    // fetch + cooldown at a time) keeps the heartbeat fresh while it works; only
    // a genuinely wedged step lets it age out. Margin: 3 ticks, or two full fetch
    // timeouts, whichever is longer, floored at 5 minutes.
    healthStallMs: num(
      process.env.HEALTH_SEEDER_STALL_MS,
      Math.max(tickMs * 3, fetchTimeoutMs * 2, 300000),
    ),
  };
}
...
// --- liveness heartbeat (read by health(), served at /health) ----------------
let lastBeatAt = 0; // last time the loop demonstrably made progress
let scanStartedAt = 0; // when the in-progress cluster scan began (0 = none)

// Mark progress. Called at the top of every tick and at each checkpoint inside a
// scan, so "the loop is alive" and "a running scan is advancing" are the same
// signal — a wedged fetch or diff batch ages it out just like a dead tick chain.
function beat() {
  lastBeatAt = nowFn();
}
...
async function collect(rootUrl, hooks) {
  ...
  while (stack.length) {
    ...
    beat(); // ...and liveness checkpoint: this scan is still advancing
...
async function scanSitemap(sm, token) {
  ...
  try {
    ...
    const wasAborted = !!(e && e.code === 'ABORTED');
    // A shutdown abort is routine (deploys land mid-scan) — never surface it
    // as the sitemap's lastError on the dashboard.
    if (!wasAborted) {
      await safely(() =>
        recordFn(sm.id, {
    ...
    util.log(
      '[seeder]',
      JSON.stringify({
        evt: wasAborted ? 'sitemap-abort' : 'sitemap-error',
        id: sm.id,
        url: sm.url,
        err: e && e.message,
      }),
    );
    return { ok: false, aborted: wasAborted };
  }
}

// One gate/lock/scan attempt. Cheap when nothing is due.
async function tickOnce() {
  beat(); // proof the setTimeout chain is still alive
  ...
  try {
    ...
    for (const sm of active) {
      ...
      // Pace BETWEEN sitemaps too — incl. after a failed root, where collect()
      // throws before its own cooldown. The Python manager's shared-stack
      // collect cooled down after EVERY fetch attempt; without this, N
      // configured sitemaps (or N failing roots when the origin is already
      // struggling) would be hit back-to-back.
      if (!first && cfg.cooldownMs > 0) {
        await sleepFn(cfg.cooldownMs);
        if (shuttingDown()) {
          summary.aborted = true;
          break;
        }
      ...
      if (r.ok) {
        ...
        // A deploy mid-scan is not a sitemap failure — don't count it.
        summary.aborted = true;
        break;
      } else {
        summary.errors += 1;
...
// --- deep liveness ----------------------------------------------------------
// `armed:false` when this instance doesn't run the seeder (the on-demand tier,
// or REFRESHER_MODE without CACHE_ENABLED) or once it's stopped for shutdown —
// health.js skips it rather than treating "never started" as "broken".
function health(now) {
  if (!cfg || stopped) return { armed: false };
  const t = now || nowFn();
  const beatAgeMs = lastBeatAt ? t - lastBeatAt : null;
  const failing = [];
  if (beatAgeMs == null || beatAgeMs > cfg.healthStallMs)
  ...
  return {
    ...
    failing,
    beatAgeMs,
    scanning: scanStartedAt > 0,
    scanAgeMs: scanStartedAt ? t - scanStartedAt : null,
    thresholds: { stallMs: cfg.healthStallMs },
  };
}
...
function start(srv, deps) {
  ...
  // Seed the heartbeat so /health can't read "stalled" in the window between
  // start() and the first tick (schedule(0) is still a macrotask away).
  scanStartedAt = 0;
  beat();
  util.log(
    '[seeder] started',
