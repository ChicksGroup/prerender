...
util.getUrl = function (requestedUrl) {
  ...
  // Ignore all URL query params: a URL and the same URL with any query string
  // should render and cache as a single page. We drop the query string here so
  // it never reaches the cache key or the headless browser. The hashbang
  // fragment derived from _escaped_fragment_ above is preserved.
  parts.query = {};
  parts.search = null;
...
// Comma-separated domain list (an env var) -> normalized array: lowercased,
// trimmed, empties dropped.
util.parseDomainList = function (v) {
  return (v || '')
    .split(',')
...
// Exact host OR any subdomain of a listed base domain.
//   ['chicksx.com'] matches chicksx.com, www.chicksx.com, api.chicksx.com
//   but NOT evilchicksx.com or chicksx.com.attacker.com
util.hostMatchesDomains = function (hostname, domains) {
